import { bigintToMinor, calendarToDateColumn, lastNDays, type CalendarDate, type Minor } from "@ledgeriq/shared";
...
export interface DashboardView {
  ...
  last30Days: { from: CalendarDate; to: CalendarDate; incomeMinor: Minor; expenseMinor: Minor; transactions: TransactionView[] };
...
export function dashboardService(db: Db) {
  ...
  return {
    async get(organizationId: string, currency: string, timezone: string, now: Date = new Date()): Promise<DashboardView> {
      ...
      const [totalIncome, totalExpense, income30, expense30, income60, cashAgg, uncategorizedCount, recent, in60, all30] = await Promise.all([
      ...
      return {
        ...
        uncategorizedCount,
        last30Days: { from: d30.from, to: d30.to, incomeMinor: income30, expenseMinor: expense30, transactions: all30.data },
